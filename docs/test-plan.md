...
| TC-05 | update_status | **Note:** `examples/update_status.json` uses `id:"123-off"` — before running, confirm this id actually exists in the fixture, or replace it with a real existing id. Back up the fixture first. | `examples/update_status.json` → `{id:"<existing id>", new_status:"interview"}` | Returns the updated application as JSON with its `status` changed to `"interview"`. | | |
...
| TC-10 | get_next_actions | Same setup | `{"limit":50}` | Zod rejects the input because `limit` must be at most 10. | | |
...
| TC-12 | list_applications | Back up the fixture, then temporarily rename `data/applications.json` to simulate an unavailable/offline local data source. | `{}` | Returns the short generic error `"Could not read applications data."` without a stack trace or internal details. | | |
